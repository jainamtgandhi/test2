// Launch Plans Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const launchplansplaceholderImage = document.getElementById('launchplansplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!launchplansplaceholderImage || !appRoot) {
    ...
    launchplansplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            launchplansplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            launchplansplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = launchplansplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = launchplansplaceholderImage.naturalWidth / rect.width;
            const scaleY = launchplansplaceholderImage.naturalHeight / rect.height;
    ...
    launchplansplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        launchplansplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            launchplansplaceholderImage.classList.remove('dragging');
        }
    });
