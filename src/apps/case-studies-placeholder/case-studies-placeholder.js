// Case Studies Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const casestudiesplaceholderImage = document.getElementById('casestudiesplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!casestudiesplaceholderImage || !appRoot) {
    ...
    casestudiesplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            casestudiesplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            casestudiesplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = casestudiesplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = casestudiesplaceholderImage.naturalWidth / rect.width;
            const scaleY = casestudiesplaceholderImage.naturalHeight / rect.height;
    ...
    casestudiesplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        casestudiesplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            casestudiesplaceholderImage.classList.remove('dragging');
        }
    });
