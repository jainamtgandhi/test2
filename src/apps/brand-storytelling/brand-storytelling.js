// Brand Storytelling App
document.addEventListener('DOMContentLoaded', function() {
    const brandstorytellingImage = document.getElementById('brandstorytellingImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!brandstorytellingImage || !appRoot) {
    ...
    brandstorytellingImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            brandstorytellingImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            brandstorytellingImage.classList.add('zoomed');
            
            // Center on click position
            const rect = brandstorytellingImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = brandstorytellingImage.naturalWidth / rect.width;
            const scaleY = brandstorytellingImage.naturalHeight / rect.height;
    ...
    brandstorytellingImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        brandstorytellingImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            brandstorytellingImage.classList.remove('dragging');
        }
    });
