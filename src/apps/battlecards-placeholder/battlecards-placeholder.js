// Battlecards Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const battlecardsplaceholderImage = document.getElementById('battlecardsplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!battlecardsplaceholderImage || !appRoot) {
    ...
    battlecardsplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            battlecardsplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            battlecardsplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = battlecardsplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = battlecardsplaceholderImage.naturalWidth / rect.width;
            const scaleY = battlecardsplaceholderImage.naturalHeight / rect.height;
    ...
    battlecardsplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        battlecardsplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            battlecardsplaceholderImage.classList.remove('dragging');
        }
    });
