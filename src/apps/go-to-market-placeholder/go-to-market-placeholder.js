// Go To Market Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const gotomarketplaceholderImage = document.getElementById('gotomarketplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!gotomarketplaceholderImage || !appRoot) {
    ...
    gotomarketplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            gotomarketplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            gotomarketplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = gotomarketplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = gotomarketplaceholderImage.naturalWidth / rect.width;
            const scaleY = gotomarketplaceholderImage.naturalHeight / rect.height;
    ...
    gotomarketplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        gotomarketplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            gotomarketplaceholderImage.classList.remove('dragging');
        }
    });
