...
document.addEventListener('DOMContentLoaded', function() {
    const messagetestingImage = document.getElementById('messagetestingImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!messagetestingImage || !appRoot) {
    ...
    messagetestingImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            messagetestingImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            messagetestingImage.classList.add('zoomed');
            
            // Center on click position
            const rect = messagetestingImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = messagetestingImage.naturalWidth / rect.width;
            const scaleY = messagetestingImage.naturalHeight / rect.height;
    ...
    messagetestingImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        messagetestingImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            messagetestingImage.classList.remove('dragging');
        }
    });
