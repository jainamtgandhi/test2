// Content Marketing Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const contentmarketingplaceholderImage = document.getElementById('contentmarketingplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!contentmarketingplaceholderImage || !appRoot) {
    ...
    contentmarketingplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            contentmarketingplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            contentmarketingplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = contentmarketingplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = contentmarketingplaceholderImage.naturalWidth / rect.width;
            const scaleY = contentmarketingplaceholderImage.naturalHeight / rect.height;
    ...
    contentmarketingplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        contentmarketingplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            contentmarketingplaceholderImage.classList.remove('dragging');
        }
    });
