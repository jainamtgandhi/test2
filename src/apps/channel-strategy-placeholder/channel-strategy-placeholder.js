// Channel Strategy Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const channelstrategyplaceholderImage = document.getElementById('channelstrategyplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!channelstrategyplaceholderImage || !appRoot) {
    ...
    channelstrategyplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            channelstrategyplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            channelstrategyplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = channelstrategyplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = channelstrategyplaceholderImage.naturalWidth / rect.width;
            const scaleY = channelstrategyplaceholderImage.naturalHeight / rect.height;
    ...
    channelstrategyplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        channelstrategyplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            channelstrategyplaceholderImage.classList.remove('dragging');
        }
    });
