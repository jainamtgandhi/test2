// Analytics Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const analyticsplaceholderImage = document.getElementById('analyticsplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!analyticsplaceholderImage || !appRoot) {
    ...
    analyticsplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            analyticsplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            analyticsplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = analyticsplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = analyticsplaceholderImage.naturalWidth / rect.width;
            const scaleY = analyticsplaceholderImage.naturalHeight / rect.height;
    ...
    analyticsplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        analyticsplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            analyticsplaceholderImage.classList.remove('dragging');
        }
    });
