// One Pagers Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const onepagersplaceholderImage = document.getElementById('onepagersplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!onepagersplaceholderImage || !appRoot) {
    ...
    onepagersplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            onepagersplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            onepagersplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = onepagersplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = onepagersplaceholderImage.naturalWidth / rect.width;
            const scaleY = onepagersplaceholderImage.naturalHeight / rect.height;
    ...
    onepagersplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        onepagersplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            onepagersplaceholderImage.classList.remove('dragging');
        }
    });
