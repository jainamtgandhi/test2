// Social Media Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const socialmediaplaceholderImage = document.getElementById('socialmediaplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!socialmediaplaceholderImage || !appRoot) {
    ...
    socialmediaplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            socialmediaplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            socialmediaplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = socialmediaplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = socialmediaplaceholderImage.naturalWidth / rect.width;
            const scaleY = socialmediaplaceholderImage.naturalHeight / rect.height;
    ...
    socialmediaplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        socialmediaplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            socialmediaplaceholderImage.classList.remove('dragging');
        }
    });
