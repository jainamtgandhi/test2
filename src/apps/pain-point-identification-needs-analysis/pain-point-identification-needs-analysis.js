// Pain Point Identification Needs Analysis App
document.addEventListener('DOMContentLoaded', function() {
    const painpointidentificationneedsanalysisImage = document.getElementById('painpointidentificationneedsanalysisImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!painpointidentificationneedsanalysisImage || !appRoot) {
    ...
    painpointidentificationneedsanalysisImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            painpointidentificationneedsanalysisImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            painpointidentificationneedsanalysisImage.classList.add('zoomed');
            
            // Center on click position
            const rect = painpointidentificationneedsanalysisImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = painpointidentificationneedsanalysisImage.naturalWidth / rect.width;
            const scaleY = painpointidentificationneedsanalysisImage.naturalHeight / rect.height;
    ...
    painpointidentificationneedsanalysisImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        painpointidentificationneedsanalysisImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            painpointidentificationneedsanalysisImage.classList.remove('dragging');
        }
    });
