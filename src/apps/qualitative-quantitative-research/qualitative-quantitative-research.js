// Qualitative Quantitative Research App
document.addEventListener('DOMContentLoaded', function() {
    const qualitativequantitativeresearchImage = document.getElementById('qualitativequantitativeresearchImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!qualitativequantitativeresearchImage || !appRoot) {
    ...
    qualitativequantitativeresearchImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            qualitativequantitativeresearchImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            qualitativequantitativeresearchImage.classList.add('zoomed');
            
            // Center on click position
            const rect = qualitativequantitativeresearchImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = qualitativequantitativeresearchImage.naturalWidth / rect.width;
            const scaleY = qualitativequantitativeresearchImage.naturalHeight / rect.height;
    ...
    qualitativequantitativeresearchImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        qualitativequantitativeresearchImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            qualitativequantitativeresearchImage.classList.remove('dragging');
        }
    });
