// Sales Decks Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const salesdecksplaceholderImage = document.getElementById('salesdecksplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!salesdecksplaceholderImage || !appRoot) {
    ...
    salesdecksplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            salesdecksplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            salesdecksplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = salesdecksplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = salesdecksplaceholderImage.naturalWidth / rect.width;
            const scaleY = salesdecksplaceholderImage.naturalHeight / rect.height;
    ...
    salesdecksplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        salesdecksplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            salesdecksplaceholderImage.classList.remove('dragging');
        }
    });
