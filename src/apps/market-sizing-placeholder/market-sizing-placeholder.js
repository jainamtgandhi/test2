// Market Sizing Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const marketsizingplaceholderImage = document.getElementById('marketsizingplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!marketsizingplaceholderImage || !appRoot) {
    ...
    marketsizingplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            marketsizingplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            marketsizingplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = marketsizingplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = marketsizingplaceholderImage.naturalWidth / rect.width;
            const scaleY = marketsizingplaceholderImage.naturalHeight / rect.height;
    ...
    marketsizingplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        marketsizingplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            marketsizingplaceholderImage.classList.remove('dragging');
        }
    });
