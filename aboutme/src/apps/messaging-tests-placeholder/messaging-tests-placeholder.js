// Messaging Tests Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const messagingtestsplaceholderImage = document.getElementById('messagingtestsplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!messagingtestsplaceholderImage || !appRoot) {
    ...
    messagingtestsplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            messagingtestsplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            messagingtestsplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = messagingtestsplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = messagingtestsplaceholderImage.naturalWidth / rect.width;
            const scaleY = messagingtestsplaceholderImage.naturalHeight / rect.height;
    ...
    messagingtestsplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        messagingtestsplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            messagingtestsplaceholderImage.classList.remove('dragging');
        }
    });
