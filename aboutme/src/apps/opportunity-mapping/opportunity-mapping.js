// Opportunity Mapping App
document.addEventListener('DOMContentLoaded', function() {
    const opportunitymappingImage = document.getElementById('opportunitymappingImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!opportunitymappingImage || !appRoot) {
    ...
    opportunitymappingImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            opportunitymappingImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            opportunitymappingImage.classList.add('zoomed');
            
            // Center on click position
            const rect = opportunitymappingImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = opportunitymappingImage.naturalWidth / rect.width;
            const scaleY = opportunitymappingImage.naturalHeight / rect.height;
    ...
    opportunitymappingImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        opportunitymappingImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            opportunitymappingImage.classList.remove('dragging');
        }
    });
