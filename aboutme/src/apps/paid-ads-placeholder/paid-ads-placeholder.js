// Paid Ads Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const paidadsplaceholderImage = document.getElementById('paidadsplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!paidadsplaceholderImage || !appRoot) {
    ...
    paidadsplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            paidadsplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            paidadsplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = paidadsplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = paidadsplaceholderImage.naturalWidth / rect.width;
            const scaleY = paidadsplaceholderImage.naturalHeight / rect.height;
    ...
    paidadsplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        paidadsplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            paidadsplaceholderImage.classList.remove('dragging');
        }
    });
