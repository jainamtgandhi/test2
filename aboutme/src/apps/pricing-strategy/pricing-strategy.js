...
document.addEventListener('DOMContentLoaded', function() {
    const pricingstrategyImage = document.getElementById('pricingstrategyImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!pricingstrategyImage || !appRoot) {
    ...
    pricingstrategyImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            pricingstrategyImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            pricingstrategyImage.classList.add('zoomed');
            
            // Center on click position
            const rect = pricingstrategyImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = pricingstrategyImage.naturalWidth / rect.width;
            const scaleY = pricingstrategyImage.naturalHeight / rect.height;
    ...
    pricingstrategyImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        pricingstrategyImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            pricingstrategyImage.classList.remove('dragging');
        }
    });
