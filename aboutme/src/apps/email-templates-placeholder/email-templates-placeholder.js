// Email Templates Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const emailtemplatesplaceholderImage = document.getElementById('emailtemplatesplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!emailtemplatesplaceholderImage || !appRoot) {
    ...
    emailtemplatesplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            emailtemplatesplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            emailtemplatesplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = emailtemplatesplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = emailtemplatesplaceholderImage.naturalWidth / rect.width;
            const scaleY = emailtemplatesplaceholderImage.naturalHeight / rect.height;
    ...
    emailtemplatesplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        emailtemplatesplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            emailtemplatesplaceholderImage.classList.remove('dragging');
        }
    });
