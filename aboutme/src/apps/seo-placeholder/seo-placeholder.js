...
document.addEventListener('DOMContentLoaded', function() {
    const seoplaceholderImage = document.getElementById('seoplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!seoplaceholderImage || !appRoot) {
    ...
    seoplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            seoplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            seoplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = seoplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = seoplaceholderImage.naturalWidth / rect.width;
            const scaleY = seoplaceholderImage.naturalHeight / rect.height;
    ...
    seoplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        seoplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            seoplaceholderImage.classList.remove('dragging');
        }
    });
