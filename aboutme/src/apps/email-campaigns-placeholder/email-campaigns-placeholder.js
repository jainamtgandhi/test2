// Email Campaigns Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const emailcampaignsplaceholderImage = document.getElementById('emailcampaignsplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!emailcampaignsplaceholderImage || !appRoot) {
    ...
    emailcampaignsplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            emailcampaignsplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            emailcampaignsplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = emailcampaignsplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = emailcampaignsplaceholderImage.naturalWidth / rect.width;
            const scaleY = emailcampaignsplaceholderImage.naturalHeight / rect.height;
    ...
    emailcampaignsplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        emailcampaignsplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            emailcampaignsplaceholderImage.classList.remove('dragging');
        }
    });
