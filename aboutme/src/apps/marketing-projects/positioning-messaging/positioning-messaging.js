...
document.addEventListener('DOMContentLoaded', async function() {
    console.log('Customer Research 2: DOM Content Loaded');
    
    // Load content from info.json
    let info = null;
    try {
        console.log('Attempting to fetch info.json...');
    ...
    folderItems.forEach(folder => {
        folder.addEventListener('click', function() {
            ...
            console.log('Folder clicked:', folderName, folderLabel);
            console.log('Folder element:', this);
            console.log('Data folder attribute:', this.getAttribute('data-folder'));
...
function openFolder(folderName, folderLabel) {
    console.log('openFolder called with:', folderName, folderLabel);
    console.log('Window parent:', window.parent);
    console.log('Event bus:', window.parent?.eventBus);
    console.log('EVENTS:', window.parent?.EVENTS);
    
    // Use the event bus to open the folder as a new program
    if (window.parent && window.parent.eventBus && window.parent.EVENTS) {
        console.log('Publishing PROGRAM_OPEN event for:', folderName);
        try {
        ...
        console.error('Event bus not available:', {
            ...
            hasEventBus: !!(window.parent && window.parent.eventBus),
            hasEvents: !!(window.parent && window.parent.EVENTS)
        });
    }
...
document.addEventListener('DOMContentLoaded', function() {
    updateStatusBar('4 objects | Contains value proposition, messaging frameworks, brand storytelling, and message testing');
});
