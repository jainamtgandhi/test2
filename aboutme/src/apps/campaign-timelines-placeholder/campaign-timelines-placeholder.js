// Campaign Timelines Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const campaigntimelinesplaceholderImage = document.getElementById('campaigntimelinesplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!campaigntimelinesplaceholderImage || !appRoot) {
    ...
    campaigntimelinesplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            campaigntimelinesplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            campaigntimelinesplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = campaigntimelinesplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = campaigntimelinesplaceholderImage.naturalWidth / rect.width;
            const scaleY = campaigntimelinesplaceholderImage.naturalHeight / rect.height;
    ...
    campaigntimelinesplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        campaigntimelinesplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            campaigntimelinesplaceholderImage.classList.remove('dragging');
        }
    });
