// Tone Voice Placeholder App
document.addEventListener('DOMContentLoaded', function() {
    const tonevoiceplaceholderImage = document.getElementById('tonevoiceplaceholderImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!tonevoiceplaceholderImage || !appRoot) {
    ...
    tonevoiceplaceholderImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            tonevoiceplaceholderImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            tonevoiceplaceholderImage.classList.add('zoomed');
            
            // Center on click position
            const rect = tonevoiceplaceholderImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = tonevoiceplaceholderImage.naturalWidth / rect.width;
            const scaleY = tonevoiceplaceholderImage.naturalHeight / rect.height;
    ...
    tonevoiceplaceholderImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        tonevoiceplaceholderImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            tonevoiceplaceholderImage.classList.remove('dragging');
        }
    });
