...
document.addEventListener('DOMContentLoaded', function() {
    const marketsizingImage = document.getElementById('marketsizingImage');
    const appRoot = document.getElementById('appRoot');
    
    if (!marketsizingImage || !appRoot) {
    ...
    marketsizingImage.addEventListener('click', function(e) {
        e.preventDefault();
        ...
        if (isZoomed) {
            // Zoom out
            marketsizingImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            marketsizingImage.classList.add('zoomed');
            
            // Center on click position
            const rect = marketsizingImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = marketsizingImage.naturalWidth / rect.width;
            const scaleY = marketsizingImage.naturalHeight / rect.height;
    ...
    marketsizingImage.addEventListener('mousedown', function(e) {
        if (!isZoomed) return;
        
        isDragging = true;
        marketsizingImage.classList.add('dragging');
    ...
    document.addEventListener('mouseup', function() {
        if (isDragging) {
            isDragging = false;
            marketsizingImage.classList.remove('dragging');
        }
    });
