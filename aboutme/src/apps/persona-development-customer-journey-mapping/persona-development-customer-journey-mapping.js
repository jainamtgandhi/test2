...
document.addEventListener('DOMContentLoaded', function() {
    ...
    personaImage.addEventListener('click', function(e) {
        ...
        if (isZoomed) {
            // Zoom out
            personaImage.classList.remove('zoomed');
            appRoot.scrollTo({ left: 0, top: 0, behavior: 'smooth' });
        } else {
            // Zoom in
            personaImage.classList.add('zoomed');
            
            // Center on click position
            const rect = personaImage.getBoundingClientRect();
            const clickX = e.clientX - rect.left;
            const clickY = e.clientY - rect.top;
            
            const scaleX = personaImage.naturalWidth / rect.width;
            const scaleY = personaImage.naturalHeight / rect.height;
